...
|----|------|----------|----|----|----|----|----|----|
| q1 | 父块还原 | PASS（但只有子块片段） | 同 v0 | | | | | |
| q2 | FAQ | PASS | 同 v0 | | | | | |
| q3 | 重叠拼接 | MISS | 同 v0 | | | | | |
| q4 | 短块扩展 | PASS（侥幸双命中） | 同 v0 | | | | | |
| q5 | MMR 去冗余 | MISS（检回旧版漏掉新版） | 同 v0 | | | | | |
| q6 | 关键词命中 | PASS*（mock 即词面匹配） | 同 v0 | | | | | |
| q7 | 语义命中 | MISS（词面零重叠） | 同 v0 | | | | | |
| q8 | 历史依赖 | MISS（不读历史必跑偏） | 同 v0 | | | | | |
| q9 | 关键词命中 | PASS* | 同 v0 | | | | | |
