...
- [x] v3_hybrid_fusion（2026-07-24 完成。两路并行→按引擎归一化（内存引擎=
  原始余弦，与 Milvus 同组走 (s+1)/2）→RRF（k=60,0.7/0.3，1-indexed 首次名次、
  向量路元数据优先）→单路退化去重→低召回扩展（四种变体、并发 16、关键词阈值
  ×0.8、Go 字节长度语义）。mock 评测 7/9：q7 被两路弱信号 RRF 叠加救回；q5 的
  近重复对首次同时进入候选池（第 3/5 名）——v4 的问题由此真实产生。课程简化：
  扩展命中尾部追加（量纲不可比，真实系统靠精排统一重打分），关键词阈值 5.0 为
  本语料实测调参（真命中 6-19、噪声 2-5））
- [x] v4_rerank_mmr（2026-07-24 完成。CHUNK_RERANK 事件：mock/真实 cross-encoder
  → 阈值 0.2 过滤 + top1 兜底 ≥0.15 + 降级重试（×0.7 下限 0.3）→ 复合分数
  0.6/0.3/0.1×位置先验（rerank.go:439-460，位置先验是 SPEC 外的实现细节）→
  MMR λ=0.7（冗余度=与已选的最大 Jaccard，bigram 近似 jieba）。mock 评测 7/9：
  q5 翻正（主要是复合分数功劳——语料近重复是改写式，Jaccard 0.198，MMR 罚不动；
  复制式场景由单测演示，λ 敏感性 0.7→第3/0.3→第5 有实测表）；q8 排序债还清；
  q4 诚实回归（sdk-03 模型分 0.158<0.2 被滤——v5 短块扩展不依赖它被检索到）。
  代码考古：top1 兜底让降级重试几乎成死路径，仅模型返空列表时触发，有测试钉住）
- [x] v5_merge（2026-07-24 完成。CHUNK_MERGE 八步流水（merge.go:44-96）：
  选输入→去重(id+MD5签名)→历史注入(Jaccard≥0.15/×0.6/≤3)→父块还原(text 分支)→
  分组重叠拼接(文本匹配缝合，minOverlap=12/窗口400)→FAQ 格式化→短块扩展
  (350→850 rune)→再拼接→终去重+0.85 部分重叠剔除；FILTER_TOP_K 确定性排序
  截断。评测判定升级为覆盖命中（sub_chunk_ids 账本，对照 SubChunkID；q8 历史
  补 retrieved_chunk_ids 字段）。mock 8/9：q4 治愈（sdk-03 作为邻居拼回，
  不依赖检索命中）；q1 还原 822 字父块；q3 剩最后半步（mock cross-encoder
